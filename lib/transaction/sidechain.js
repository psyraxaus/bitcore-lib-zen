...
class Vsc_ccout {
    constructor (scid, epoch_length, satoshis, satoshisBN, address, customData) {
        this.scid = scid;
...
Sidechain.prototype._fromObject = function(params) {
  ...
  _.each(params.vsc_ccout, function (vsc_ccout){
    vsc.push(new Vsc_ccout(BufferUtil.reverse(new buffer.Buffer(vsc_ccout.scid, 'hex')),vsc_ccout.epoch_length,vsc_ccout.satoshis,vsc_ccout.satoshisBN,BufferUtil.reverse(new buffer.Buffer(vsc_ccout.address, 'hex')),new Buffer(vsc_ccout.customData)));
   });
  this.vsc_ccout = vsc;
...
Sidechain.fromBufferReader = function(br) {
  ...
  for (i = 0; i < n_vsc; i++) {
    ...
    var vsc = new Vsc_ccout(scid, withdrawal_epoch_length, satoshis, satoshisBN, address, customData);
