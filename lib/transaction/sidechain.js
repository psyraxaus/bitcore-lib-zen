...
class Vsc_ccout {
    constructor (epoch_length, satoshis, satoshisBN, address, customData) {
...
Sidechain.prototype._fromObject = function(params) {
  ...
  _.each(params.vsc_ccout, function (vsc_ccout){
    vsc.push(new Vsc_ccout(vsc_ccout.epoch_length,vsc_ccout.satoshis,vsc_ccout.satoshisBN,BufferUtil.reverse(new buffer.Buffer(vsc_ccout.address, 'hex')),new Buffer(vsc_ccout.customData, 'hex')));
   });
  this.vsc_ccout = vsc;
...
Sidechain.fromBufferReader = function(br) {
  ...
  for (i = 0; i < n_vsc; i++) {
    ...
    var vsc = new Vsc_ccout(withdrawal_epoch_length, satoshis, satoshisBN, address, customData);
