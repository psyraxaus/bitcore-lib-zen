...
describe('PrivateKey', function() {
  ...
  it('should create a private key from WIF string', function() {
    var a = new PrivateKey('L1DVMHqVxanR9sgCFE85yEwocdYFuAm1TmLNB6bN6fkp4KkDy9Av');
    should.exist(a);
    should.exist(a.bn);
  });

  it('should create a private key from WIF buffer', function() {
    var a = new PrivateKey(Base58Check.decode('L1DVMHqVxanR9sgCFE85yEwocdYFuAm1TmLNB6bN6fkp4KkDy9Av'));
    should.exist(a);
    should.exist(a.bn);
  });
  ...
  describe('instantiation', function() {
    ...
    it('should not be able to instantiate private key because of network mismatch', function() {
      expect(function() {
        return new PrivateKey('L1DVMHqVxanR9sgCFE85yEwocdYFuAm1TmLNB6bN6fkp4KkDy9Av', 'testnet');
      }).to.throw('Private key network mismatch');
    });

    it('should not be able to instantiate private key WIF is too long', function() {
      expect(function() {
        var buf = Base58Check.decode('L1DVMHqVxanR9sgCFE85yEwocdYFuAm1TmLNB6bN6fkp4KkDy9Av');
  ...
  describe('#toAddress', function() {
    it('should output this known livenet address correctly', function() {
      ...
      address.toString().should.equal('znaBpsSX8S4Ld3LfsDAJPnXiZESXGjujgPQ');
    });

    it('should output this known testnet address correctly', function() {
      var privkey = PrivateKey.fromWIF('cR4qogdN9UxLZJXCNFNwDRRZNeLRWuds9TTSuLNweFVjiaE4gPaq');
      var address = privkey.toAddress();
      address.toString().should.equal('ztgA9FPaX7ZA88uFYvLGhejcFT6GZnnjwtR');
    });
  ...
  it('creates an address as expected from WIF, livenet', function() {
    var privkey = new PrivateKey('KwFist9hv34Y2TRByaXSoJc8gnz9J1EK41ezYcwhZzaaAkLrnUsS');
    privkey.publicKey.toAddress().toString().should.equal('znc4SAjxJDQAvkdiUkS9Ru2SNV5jUTGHViw');
  });

  it('creates an address as expected from WIF, testnet', function() {
    var privkey = new PrivateKey('cPrh1yrAAwFQeyS1H3cGhEeeujY91uYV4yFmiRRPhEDeMLvPJVw9');
    privkey.publicKey.toAddress().toString().should.equal('ztdbpAdAQQpodWBEPYggAruWZsqv1rory6J');
  });
